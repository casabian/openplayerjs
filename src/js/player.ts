...
class Player {
	...
	public static init(): void {
		...
		for (let i = 0, total = targets.length; i < total; i++) {
			...
			const player = new Player(
				target,
				target.getAttribute('data-op-ads'),
				!!target.getAttribute('data-op-fill'),
				JSON.parse(target.getAttribute('data-op-options'))
			);
			player.init();
		}
	}
	...
	private _createPlayButton(): void {
		if (isAudio(this.element) || this.options.notWrapInstance) {
			return;
		}
